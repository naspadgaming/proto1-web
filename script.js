...
document.getElementById("btnCV").onclick = () => {
  ...
  if (cv.fileName) {
    cvPreview.textContent = "File tersimpan: " + cv.fileName;
  } else {
    cvPreview.textContent = "Belum ada file CV.";
  }

  modalCV.style.display = "flex";
};

document.getElementById("cancelCV").onclick = () => {
...
document.getElementById("saveCV").onclick = () => {
  ...
  const skill = cvSkill.value.trim();
  ...
  if (fileInput.files.length > 0) {
    ...
    reader.onload = function () {
      ...
      cvPreview.textContent = "File tersimpan: " + fileName;
      modalCV.style.display = "none";
      alert("CV berhasil disimpan!");
    };
    reader.readAsDataURL(file); // simpan file sebagai base64
  } 
  else {
    // Jika hanya mengupdate teks tanpa file
    const cvOld = JSON.parse(localStorage.getItem(LS_CV) || "{}");
    const data = {
      name,
      email,
      skill,
      fileName: cvOld.fileName || null,
      fileData: cvOld.fileData || null
    };
